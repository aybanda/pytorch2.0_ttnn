...
class AtenModule(torch.nn.Module):
    ...
    def forward(self, *args, **kwargs):
        return torch.ops.aten.detach.default(*args, **kwargs)
...
def teardown_module(module):
    print(metrics)
    save_pickle(metrics, "metrics-autogen-op/microsoft/beit-large-patch16-224-train", "aten.detach.default")


@pytest.mark.parametrize("input_strings", [["Tensor<[1, 16, 197, 197]> self = ?"]])
def test_aten(device, input_strings, input_var_only_native, input_var_check_accu, input_var_check_ttnn):
    metric = {
        "opname": "aten.detach.default",
    ...
    input_args, input_kwargs, status = render_metric_string_list_to_input_args_kwargs(
        "aten.detach.default", input_strings
    )
    if status == False:
