...
class AtenModule(torch.nn.Module):
    ...
    def forward(self, *args, **kwargs):
        return torch.ops.aten.elu_backward.default(*args, **kwargs)
...
def teardown_module(module):
    print(metrics)
    save_pickle(metrics, "metrics-autogen-op/OpenPose V2-train", "aten.elu_backward.default")
...
@pytest.mark.parametrize(
    ...
    [
        [
            ...
            "number alpha = 1.0",
            "number scale = 1",
            "number input_scale = 1",
            "bool is_result = True",
            "Tensor<[1, 128, 28, 28]> self_or_result = ?",
        ]
    ],
)
def test_aten(device, input_strings, input_var_only_native, input_var_check_accu, input_var_check_ttnn):
    metric = {
        "opname": "aten.elu_backward.default",
    ...
    input_args, input_kwargs, status = render_metric_string_list_to_input_args_kwargs(
        "aten.elu_backward.default", input_strings
    )
    if status == False:
