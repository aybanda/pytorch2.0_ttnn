...
class AtenModule(torch.nn.Module):
    ...
    def forward(self, *args, **kwargs):
        return torch.ops.aten.elu.default(*args, **kwargs)
...
def teardown_module(module):
    print(metrics)
    save_pickle(metrics, "metrics-autogen-op/OpenPose V2-train", "aten.elu.default")


@pytest.mark.parametrize("input_strings", [["Tensor<[1, 128, 28, 28]> self = ?", "number alpha = 1.0"]])
def test_aten(device, input_strings, input_var_only_native, input_var_check_accu, input_var_check_ttnn):
    metric = {
        "opname": "aten.elu.default",
    ...
    m = AtenModule()
    input_args, input_kwargs, status = render_metric_string_list_to_input_args_kwargs("aten.elu.default", input_strings)
    if status == False:
