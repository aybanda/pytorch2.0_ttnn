...
class AtenModule(torch.nn.Module):
    ...
    def forward(self, *args, **kwargs):
        return torch.ops.aten.full_like.default(*args, **kwargs)
...
def teardown_module(module):
    print(metrics)
    save_pickle(metrics, "metrics-autogen-op/FLAN-T5", "aten.full_like.default")


@pytest.mark.parametrize(
    "input_strings",
    [
        ["Tensor<[15, 15]> self = ?", "number fill_value = 15", "Optional[bool] pin_memory = False"],
        ["Tensor<[1, 1]> self = ?", "number fill_value = 31", "Optional[bool] pin_memory = False"],
...
def test_aten(device, input_strings, input_var_only_native, input_var_check_accu, input_var_check_ttnn):
    ...
    input_args, input_kwargs, status = render_metric_string_list_to_input_args_kwargs(
        "aten.full_like.default", input_strings
    )
    if status == False:
