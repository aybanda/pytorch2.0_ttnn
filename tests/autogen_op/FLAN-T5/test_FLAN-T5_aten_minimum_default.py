...
class AtenModule(torch.nn.Module):
    ...
    def forward(self, *args, **kwargs):
        return torch.ops.aten.minimum.default(*args, **kwargs)
...
def teardown_module(module):
    print(metrics)
    save_pickle(metrics, "metrics-autogen-op/FLAN-T5", "aten.minimum.default")
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ["Tensor<[2, 2]> self = ?", "Tensor<[2, 2]> other = ?"],
        ["Tensor<[s0 + 1, s0 + 1]> self = ?", "Tensor<[s0 + 1, s0 + 1]> other = ?"],
        ["Tensor<[17, 17]> self = ?", "Tensor<[17, 17]> other = ?"],
    ],
)
def test_aten(device, input_strings, input_var_only_native, input_var_check_accu, input_var_check_ttnn):
    metric = {
        "opname": "aten.minimum.default",
    ...
    input_args, input_kwargs, status = render_metric_string_list_to_input_args_kwargs(
        "aten.minimum.default", input_strings
    )
    if status == False:
