...
class AtenModule(torch.nn.Module):
    ...
    def forward(self, *args, **kwargs):
        return torch.ops.aten.baddbmm.default(*args, **kwargs)
...
def teardown_module(module):
    print(metrics)
    save_pickle(metrics, "metrics-autogen-op/DETR", "aten.baddbmm.default")


@pytest.mark.parametrize(
    "input_strings",
    [
        ["Tensor<[8, 1, 920]> self = ?", "Tensor<[8, 920, 32]> batch1 = ?", "Tensor<[8, 32, 920]> batch2 = ?"],
        ["Tensor<[8, 1, 920]> self = ?", "Tensor<[8, 100, 32]> batch1 = ?", "Tensor<[8, 32, 920]> batch2 = ?"],
    ],
)
def test_aten(device, input_strings, input_var_only_native, input_var_check_accu, input_var_check_ttnn):
    metric = {
        "opname": "aten.baddbmm.default",
    ...
    input_args, input_kwargs, status = render_metric_string_list_to_input_args_kwargs(
        "aten.baddbmm.default", input_strings
    )
    if status == False:
