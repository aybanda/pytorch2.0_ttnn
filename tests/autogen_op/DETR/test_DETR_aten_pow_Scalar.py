...
class AtenModule(torch.nn.Module):
    ...
    def forward(self, *args, **kwargs):
        return torch.ops.aten.pow.Scalar(*args, **kwargs)
...
def teardown_module(module):
    print(metrics)
    save_pickle(metrics, "metrics-autogen-op/DETR", "aten.pow.Scalar")


@pytest.mark.parametrize("input_strings", [["number self = 10000", "Tensor<[128]> exponent = ?"]])
def test_aten(device, input_strings, input_var_only_native, input_var_check_accu, input_var_check_ttnn):
    metric = {
        "opname": "aten.pow.Scalar",
    ...
    m = AtenModule()
    input_args, input_kwargs, status = render_metric_string_list_to_input_args_kwargs("aten.pow.Scalar", input_strings)
    if status == False:
