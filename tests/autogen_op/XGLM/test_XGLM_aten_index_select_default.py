...
class AtenModule(torch.nn.Module):
    ...
    def forward(self, *args, **kwargs):
        return torch.ops.aten.index_select.default(*args, **kwargs)
...
def teardown_module(module):
    print(metrics)
    save_pickle(metrics, "metrics-autogen-op/XGLM", "aten.index_select.default")


@pytest.mark.parametrize("input_strings", [["Tensor<[2050, 1024]> self = ?", "int dim = 0", "Tensor<[19]> index = ?"]])
def test_aten(device, input_strings, input_var_only_native, input_var_check_accu, input_var_check_ttnn):
    metric = {
        "opname": "aten.index_select.default",
    ...
    input_args, input_kwargs, status = render_metric_string_list_to_input_args_kwargs(
        "aten.index_select.default", input_strings
    )
    if status == False:
