...
class AtenModule(torch.nn.Module):
    ...
    def forward(self, *args, **kwargs):
        return torch.ops.aten.hardswish.default(*args, **kwargs)
...
def teardown_module(module):
    print(metrics)
    save_pickle(metrics, "metrics-autogen-op/ssdlite320_mobilenet_v3_large", "aten.hardswish.default")
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ["Tensor<[1, 240, 40, 40]> self = ?"],
        ["Tensor<[1, 240, 20, 20]> self = ?"],
        ["Tensor<[1, 200, 20, 20]> self = ?"],
        ["Tensor<[1, 184, 20, 20]> self = ?"],
        ["Tensor<[1, 480, 20, 20]> self = ?"],
        ["Tensor<[1, 672, 20, 20]> self = ?"],
        ["Tensor<[1, 672, 10, 10]> self = ?"],
        ["Tensor<[1, 480, 10, 10]> self = ?"],
    ],
)
def test_aten(device, input_strings, input_var_only_native, input_var_check_accu, input_var_check_ttnn):
    metric = {
        "opname": "aten.hardswish.default",
    ...
    input_args, input_kwargs, status = render_metric_string_list_to_input_args_kwargs(
        "aten.hardswish.default", input_strings
    )
    if status == False:
