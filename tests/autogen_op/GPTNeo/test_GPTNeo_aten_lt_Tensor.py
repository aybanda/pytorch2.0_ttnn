...
class AtenModule(torch.nn.Module):
    ...
    def forward(self, *args, **kwargs):
        return torch.ops.aten.lt.Tensor(*args, **kwargs)
...
def teardown_module(module):
    print(metrics)
    save_pickle(metrics, "metrics-autogen-op/GPTNeo", "aten.lt.Tensor")


@pytest.mark.parametrize("input_strings", [["Tensor<[1, 50257]> self = ?", "Tensor<[1, 1]> other = ?"]])
def test_aten(device, input_strings, input_var_only_native, input_var_check_accu, input_var_check_ttnn):
    ...
    m = AtenModule()
    input_args, input_kwargs, status = render_metric_string_list_to_input_args_kwargs("aten.lt.Tensor", input_strings)
    if status == False:
