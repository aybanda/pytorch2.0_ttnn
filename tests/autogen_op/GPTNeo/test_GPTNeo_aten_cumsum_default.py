...
class AtenModule(torch.nn.Module):
    ...
    def forward(self, *args, **kwargs):
        return torch.ops.aten.cumsum.default(*args, **kwargs)
...
def teardown_module(module):
    print(metrics)
    save_pickle(metrics, "metrics-autogen-op/GPTNeo", "aten.cumsum.default")


@pytest.mark.parametrize(
    "input_strings", [["Tensor<[1, 45]> self = ?", "int dim = -1"], ["Tensor<[1, s0]> self = ?", "int dim = -1"]]
)
def test_aten(device, input_strings, input_var_only_native, input_var_check_accu, input_var_check_ttnn):
    metric = {
        "opname": "aten.cumsum.default",
    ...
    input_args, input_kwargs, status = render_metric_string_list_to_input_args_kwargs(
        "aten.cumsum.default", input_strings
    )
    if status == False:
