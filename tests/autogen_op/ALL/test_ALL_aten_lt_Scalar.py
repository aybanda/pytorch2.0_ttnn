...
class AtenModule(torch.nn.Module):
    ...
    def forward(self, *args, **kwargs):
        return torch.ops.aten.lt.Scalar(*args, **kwargs)
...
def teardown_module(module):
    print(metrics)
    save_pickle(metrics, "metrics-autogen-op/ALL", "aten.lt.Scalar")


@pytest.mark.parametrize(
    "input_strings",
    [
        ["Tensor<[15, 15]> self = ?", "number other = 8"],
        ["Tensor<[1, 1]> self = ?", "number other = 16"],
        ["Tensor<[2, 2]> self = ?", "number other = 16"],
        ["Tensor<[s0 + 1, s0 + 1]> self = ?", "number other = 16"],
        ["Tensor<[17, 17]> self = ?", "number other = 16"],
        ["Tensor<[10, 10]> self = ?", "number other = 8"],
    ],
)
def test_aten(device, input_strings, input_var_only_native, input_var_check_accu, input_var_check_ttnn):
    ...
    m = AtenModule()
    input_args, input_kwargs, status = render_metric_string_list_to_input_args_kwargs("aten.lt.Scalar", input_strings)
    if status == False:
