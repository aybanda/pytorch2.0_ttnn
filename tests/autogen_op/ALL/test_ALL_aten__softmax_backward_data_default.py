...
class AtenModule(torch.nn.Module):
    ...
    def forward(self, *args, **kwargs):
        return torch.ops.aten._softmax_backward_data.default(*args, **kwargs)
...
def teardown_module(module):
    print(metrics)
    save_pickle(metrics, "metrics-autogen-op/ALL", "aten._softmax_backward_data.default")
...
@pytest.mark.parametrize(
    ...
    [
        [
            "Tensor<[16, 7, 7]> grad_output = ?",
            "Tensor<[16, 7, 7]> output = ?",
            "int dim = -1",
            "int input_dtype = torch.bfloat16",
        ],
        [
            "Tensor<[12, 50, 50]> grad_output = ?",
            "Tensor<[12, 50, 50]> output = ?",
            "int dim = -1",
            "int input_dtype = torch.bfloat16",
        ],
        [
            "Tensor<[1, 8, 256, 256]> grad_output = ?",
            "Tensor<[1, 8, 256, 256]> output = ?",
            "int dim = -1",
            "int input_dtype = torch.bfloat16",
        ],
        [
            "Tensor<[1, 5, 1024, 256]> grad_output = ?",
            "Tensor<[1, 5, 1024, 256]> output = ?",
            "int dim = -1",
            "int input_dtype = torch.bfloat16",
        ],
        [
            "Tensor<[1, 2, 4096, 256]> grad_output = ?",
            "Tensor<[1, 2, 4096, 256]> output = ?",
            "int dim = -1",
            "int input_dtype = torch.bfloat16",
        ],
        [
            "Tensor<[1, 1, 16384, 256]> grad_output = ?",
            "Tensor<[1, 1, 16384, 256]> output = ?",
            "int dim = -1",
            "int input_dtype = torch.bfloat16",
        ],
        [
            "Tensor<[1, 12, 197, 197]> grad_output = ?",
            "Tensor<[1, 12, 197, 197]> output = ?",
            "int dim = -1",
            "int input_dtype = torch.bfloat16",
        ],
        [
            "Tensor<[1, 16, 197, 197]> grad_output = ?",
            "Tensor<[1, 16, 197, 197]> output = ?",
            "int dim = -1",
            "int input_dtype = torch.bfloat16",
        ],
    ],
...
def test_aten(device, input_strings, input_var_only_native, input_var_check_accu, input_var_check_ttnn):
    metric = {
        "opname": "aten._softmax_backward_data.default",
    ...
    input_args, input_kwargs, status = render_metric_string_list_to_input_args_kwargs(
        "aten._softmax_backward_data.default", input_strings
    )
    if status == False:
