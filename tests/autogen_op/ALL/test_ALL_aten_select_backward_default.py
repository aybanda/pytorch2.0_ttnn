...
class AtenModule(torch.nn.Module):
    ...
    def forward(self, *args, **kwargs):
        return torch.ops.aten.select_backward.default(*args, **kwargs)
...
def teardown_module(module):
    print(metrics)
    save_pickle(metrics, "metrics-autogen-op/ALL", "aten.select_backward.default")


@pytest.mark.parametrize(
    "input_strings",
    [
        ["Tensor<[1, 768]> grad_output = ?", "List[int] input_sizes = [1, 50, 768]", "int dim = 1", "int index = 0"],
        ["Tensor<[1, 768]> grad_output = ?", "List[int] input_sizes = [1, 197, 768]", "int dim = 1", "int index = 0"],
    ],
)
def test_aten(device, input_strings, input_var_only_native, input_var_check_accu, input_var_check_ttnn):
    metric = {
        "opname": "aten.select_backward.default",
    ...
    input_args, input_kwargs, status = render_metric_string_list_to_input_args_kwargs(
        "aten.select_backward.default", input_strings
    )
    if status == False:
