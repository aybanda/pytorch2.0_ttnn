...
class AtenModule(torch.nn.Module):
    ...
    def forward(self, *args, **kwargs):
        return torch.ops.aten.new_ones.default(*args, **kwargs)
...
def teardown_module(module):
    print(metrics)
    save_pickle(metrics, "metrics-autogen-op/ALL", "aten.new_ones.default")


@pytest.mark.parametrize(
    "input_strings",
    [
        ["Tensor<[1, 45]> self = ?", "List[int] size = [1, 1]", "Optional[bool] pin_memory = False"],
        ["Tensor<[1, s24]> self = ?", "List[int] size = [1, 1]", "Optional[bool] pin_memory = False"],
        ["Tensor<[1, 59]> self = ?", "List[int] size = [1, 1]", "Optional[bool] pin_memory = False"],
        ["Tensor<[1, s48]> self = ?", "List[int] size = [1, 1]", "Optional[bool] pin_memory = False"],
        ["Tensor<[1, 5]> self = ?", "List[int] size = [1, 1]", "Optional[bool] pin_memory = False"],
        ["Tensor<[1, s40]> self = ?", "List[int] size = [1, 1]", "Optional[bool] pin_memory = False"],
    ],
)
def test_aten(device, input_strings, input_var_only_native, input_var_check_accu, input_var_check_ttnn):
    metric = {
        "opname": "aten.new_ones.default",
    ...
    input_args, input_kwargs, status = render_metric_string_list_to_input_args_kwargs(
        "aten.new_ones.default", input_strings
    )
    if status == False:
