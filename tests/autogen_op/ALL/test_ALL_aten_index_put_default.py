...
class AtenModule(torch.nn.Module):
    ...
    def forward(self, *args, **kwargs):
        return torch.ops.aten.index_put.default(*args, **kwargs)
...
def teardown_module(module):
    print(metrics)
    save_pickle(metrics, "metrics-autogen-op/ALL", "aten.index_put.default")
...
@pytest.mark.parametrize(
    ...
    [
        [
            "Tensor<[2, 7, 512]> self = ?",
            "List[Optional[Tensor]] indices = [<[2]>, <[2]>]",
            "Tensor<[2, 512]> values = ?",
        ...
        [
            ...
            "Tensor<[38809, 12]> values = ?",
            "bool accumulate = True",
        ],
        [
            "Tensor<[732, 16]> self = ?",
            "List[Optional[Tensor]] indices = [<[38809]>]",
            "Tensor<[38809, 16]> values = ?",
            "bool accumulate = True",
        ],
    ],
)
def test_aten(device, input_strings, input_var_only_native, input_var_check_accu, input_var_check_ttnn):
    ...
    input_args, input_kwargs, status = render_metric_string_list_to_input_args_kwargs(
        "aten.index_put.default", input_strings
    )
    if status == False:
