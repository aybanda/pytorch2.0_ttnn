...
class AtenModule(torch.nn.Module):
    ...
    def forward(self, *args, **kwargs):
        return torch.ops.aten.gelu.default(*args, **kwargs)
...
def teardown_module(module):
    print(metrics)
    save_pickle(metrics, "metrics-autogen-op/ALL", "aten.gelu.default")


@pytest.mark.parametrize(
    "input_strings",
    [
        ["Tensor<[1, 256, 4096]> self = ?"],
        ["Tensor<[1, 25, 3072]> self = ?"],
        ...
        ["Tensor<[1, 768, 3000]> self = ?"],
        ["Tensor<[1, 768, 1500]> self = ?"],
        ["Tensor<[1, 1500, 3072]> self = ?"],
        ["Tensor<[1, 1, 3072]> self = ?"],
        ["Tensor<[1, 4, 3072]> self = ?"],
        ["Tensor<[1, 19, 4096]> self = ?"],
        ["Tensor<[1, 1445, 768]> self = ?"],
        ["Tensor<[1, 16, 3072]> self = ?"],
...
def test_aten(device, input_strings, input_var_only_native, input_var_check_accu, input_var_check_ttnn):
    ...
    input_args, input_kwargs, status = render_metric_string_list_to_input_args_kwargs(
        "aten.gelu.default", input_strings
    )
    if status == False:
