...
class AtenModule(torch.nn.Module):
    ...
    def forward(self, *args, **kwargs):
        return torch.ops.aten.repeat.default(*args, **kwargs)
...
def teardown_module(module):
    print(metrics)
    save_pickle(metrics, "metrics-autogen-op/ALL", "aten.repeat.default")


@pytest.mark.parametrize(
    "input_strings",
    [
        ["Tensor<[100, 1, 256]> self = ?", "List[int] repeats = [1, 1, 1]"],
        ["Tensor<[1, 1, 1]> self = ?", "List[int] repeats = [1, 1, 1]"],
        ["Tensor<[1, 1, 2048, 2048]> self = ?", "List[int] repeats = [1, 1, 1, 1]"],
        ["Tensor<[6, 2]> self = ?", "List[int] repeats = [400, 1]"],
        ["Tensor<[6, 2]> self = ?", "List[int] repeats = [100, 1]"],
        ["Tensor<[6, 2]> self = ?", "List[int] repeats = [25, 1]"],
        ...
        ["Tensor<[4, 2]> self = ?", "List[int] repeats = [1, 1]"],
    ],
)
def test_aten(device, input_strings, input_var_only_native, input_var_check_accu, input_var_check_ttnn):
    ...
    input_args, input_kwargs, status = render_metric_string_list_to_input_args_kwargs(
        "aten.repeat.default", input_strings
    )
    if status == False:
