...
class AtenModule(torch.nn.Module):
    ...
    def forward(self, *args, **kwargs):
        return torch.ops.aten.tanh.default(*args, **kwargs)
...
def teardown_module(module):
    print(metrics)
    save_pickle(metrics, "metrics-autogen-op/ALL", "aten.tanh.default")


@pytest.mark.parametrize(
    "input_strings",
    [
        ["Tensor<[1, 32, 6144]> self = ?"],
        ["Tensor<[1, 15, 1024]> self = ?"],
        ["Tensor<[1, 1, 1024]> self = ?"],
        ["Tensor<[1, 7, 3072]> self = ?"],
        ["Tensor<[1, 45, 3072]> self = ?"],
        ["Tensor<[1, 1, 3072]> self = ?"],
        ["Tensor<[1, 9, 3072]> self = ?"],
        ["Tensor<[1, 9, 128]> self = ?"],
        ["Tensor<[1, 12, 3072]> self = ?"],
        ["Tensor<[1, 9, 4096]> self = ?"],
        ["Tensor<[1, 9, 8192]> self = ?"],
        ["Tensor<[1, 9, 16384]> self = ?"],
        ["Tensor<[1, 5, 4096]> self = ?"],
        ["Tensor<[1, 1, 4096]> self = ?"],
        ["Tensor<[1, 768]> self = ?"],
        ["Tensor<[1, 14, 3072]> self = ?"],
    ],
)
def test_aten(device, input_strings, input_var_only_native, input_var_check_accu, input_var_check_ttnn):
    ...
    input_args, input_kwargs, status = render_metric_string_list_to_input_args_kwargs(
        "aten.tanh.default", input_strings
    )
    if status == False:
