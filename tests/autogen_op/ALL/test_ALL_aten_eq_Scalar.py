...
class AtenModule(torch.nn.Module):
    ...
    def forward(self, *args, **kwargs):
        return torch.ops.aten.eq.Scalar(*args, **kwargs)
...
def teardown_module(module):
    print(metrics)
    save_pickle(metrics, "metrics-autogen-op/ALL", "aten.eq.Scalar")


@pytest.mark.parametrize(
    "input_strings",
    [
        ["Tensor<[2, 1]> self = ?", "number other = 0"],
        ["Tensor<[1, 1]> self = ?", "number other = 0"],
        ["Tensor<[1, 7]> self = ?", "number other = 1"],
        ["Tensor<[1, 7]> self = ?", "number other = 50256"],
        ...
        ["Tensor<[64, 64, 64]> self = ?", "number other = 0"],
        ["Tensor<[16, 64, 64]> self = ?", "number other = 0"],
        ["Tensor<[4, 64, 64]> self = ?", "number other = 0"],
    ],
)
def test_aten(device, input_strings, input_var_only_native, input_var_check_accu, input_var_check_ttnn):
    ...
    m = AtenModule()
    input_args, input_kwargs, status = render_metric_string_list_to_input_args_kwargs("aten.eq.Scalar", input_strings)
    if status == False:
