...
def teardown_module(module):
    print(metrics)
    save_pickle(metrics, "metrics-autogen-op/ALL", "aten.split.Tensor")


@pytest.mark.parametrize(
    "input_strings",
    [
        ["Tensor<[1, 256, 2]> self = ?", "int split_size = 1", "int dim = -1"],
        ["Tensor<[768, 256]> self = ?", "int split_size = 256"],
        ["Tensor<[768]> self = ?", "int split_size = 256"],
        ["Tensor<[1, 25, 2]> self = ?", "int split_size = 1", "int dim = -1"],
        ...
        ["Tensor<[1, 5, 4, 768]> self = ?", "int split_size = 256", "int dim = -1"],
        ["Tensor<[1, 5, 32]> self = ?", "int split_size = 16", "int dim = -1"],
        ["Tensor<[1, 1, 4, 768]> self = ?", "int split_size = 256", "int dim = -1"],
        ["Tensor<[1, 1, 32]> self = ?", "int split_size = 16", "int dim = -1"],
        ["Tensor<[1, 14, 2]> self = ?", "int split_size = 1", "int dim = -1"],
    ],
)
def test_aten(device, input_strings, input_var_only_native, input_var_check_accu, input_var_check_ttnn):
    metric = {
        "opname": "aten.split.Tensor",
    ...
    input_args, input_kwargs, status = render_metric_string_list_to_input_args_kwargs(
        "aten.split.Tensor", input_strings
    )
    if status == False:
