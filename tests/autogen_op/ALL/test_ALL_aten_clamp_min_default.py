...
class AtenModule(torch.nn.Module):
    ...
    def forward(self, *args, **kwargs):
        return torch.ops.aten.clamp_min.default(*args, **kwargs)
...
def teardown_module(module):
    print(metrics)
    save_pickle(metrics, "metrics-autogen-op/ALL", "aten.clamp_min.default")


@pytest.mark.parametrize(
    "input_strings",
    [
        ["Tensor<[1, 1, 38, 38]> self = ?", "number min = 1e-12"],
        ["Tensor<[64, 4, 64, 1]> self = ?", "number min = 1e-12"],
        ["Tensor<[16, 8, 64, 1]> self = ?", "number min = 1e-12"],
        ["Tensor<[4, 16, 64, 1]> self = ?", "number min = 1e-12"],
        ["Tensor<[1, 32, 64, 1]> self = ?", "number min = 1e-12"],
        ["Tensor<[64, 3, 64, 1]> self = ?", "number min = 1e-12"],
        ["Tensor<[16, 6, 64, 1]> self = ?", "number min = 1e-12"],
        ["Tensor<[4, 12, 64, 1]> self = ?", "number min = 1e-12"],
        ["Tensor<[1, 24, 64, 1]> self = ?", "number min = 1e-12"],
    ],
)
def test_aten(device, input_strings, input_var_only_native, input_var_check_accu, input_var_check_ttnn):
    ...
    input_args, input_kwargs, status = render_metric_string_list_to_input_args_kwargs(
        "aten.clamp_min.default", input_strings
    )
    if status == False:
