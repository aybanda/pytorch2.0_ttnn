...
class AtenModule(torch.nn.Module):
    ...
    def forward(self, *args, **kwargs):
        return torch.ops.aten.new_full.default(*args, **kwargs)
...
def teardown_module(module):
    print(metrics)
    save_pickle(metrics, "metrics-autogen-op/ALL", "aten.new_full.default")
...
@pytest.mark.parametrize(
    ...
    [
        [
            "Tensor<[3, 320, 320]> self = ?",
            "List[int] size = [1, 3, 320, 320]",
            "number fill_value = 0",
            "Optional[bool] pin_memory = False",
        ],
        [
            "Tensor<[3, 800, 1066]> self = ?",
            "List[int] size = [1, 3, 800, 1088]",
            "number fill_value = 0",
            "Optional[bool] pin_memory = False",
        ],
        [
            "Tensor<[3, 300, 300]> self = ?",
            "List[int] size = [1, 3, 300, 300]",
            "number fill_value = 0",
            "Optional[bool] pin_memory = False",
        ],
    ],
...
def test_aten(device, input_strings, input_var_only_native, input_var_check_accu, input_var_check_ttnn):
    ...
    input_args, input_kwargs, status = render_metric_string_list_to_input_args_kwargs(
        "aten.new_full.default", input_strings
    )
    if status == False:
