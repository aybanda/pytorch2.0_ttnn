...
class AtenModule(torch.nn.Module):
    ...
    def forward(self, *args, **kwargs):
        return torch.ops.aten._log_softmax.default(*args, **kwargs)
...
def teardown_module(module):
    print(metrics)
    save_pickle(metrics, "metrics-autogen-op/ALL", "aten._log_softmax.default")


@pytest.mark.parametrize(
    "input_strings",
    [
        ["Tensor<[1, 10]> self = ?", "int dim = 1", "bool half_to_float = False"],
        ["Tensor<[19, 256008]> self = ?", "int dim = 1", "bool half_to_float = False"],
    ],
)
def test_aten(device, input_strings, input_var_only_native, input_var_check_accu, input_var_check_ttnn):
    metric = {
        "opname": "aten._log_softmax.default",
    ...
    input_args, input_kwargs, status = render_metric_string_list_to_input_args_kwargs(
        "aten._log_softmax.default", input_strings
    )
    if status == False:
