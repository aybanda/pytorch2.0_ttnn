...
class AtenModule(torch.nn.Module):
    ...
    def forward(self, *args, **kwargs):
        return torch.ops.aten.ne.Scalar(*args, **kwargs)
...
def teardown_module(module):
    print(metrics)
    save_pickle(metrics, "metrics-autogen-op/ALL", "aten.ne.Scalar")


@pytest.mark.parametrize(
    "input_strings",
    [
        ["Tensor<[1, 10]> self = ?", "number other = 1"],
...
def test_aten(device, input_strings, input_var_only_native, input_var_check_accu, input_var_check_ttnn):
    ...
    m = AtenModule()
    input_args, input_kwargs, status = render_metric_string_list_to_input_args_kwargs("aten.ne.Scalar", input_strings)
    if status == False:
