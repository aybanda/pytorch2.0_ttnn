...
class AtenModule(torch.nn.Module):
    ...
    def forward(self, *args, **kwargs):
        return torch.ops.aten.leaky_relu.default(*args, **kwargs)
...
def teardown_module(module):
    print(metrics)
    save_pickle(metrics, "metrics-autogen-op/YOLOv3", "aten.leaky_relu.default")


@pytest.mark.parametrize(
    "input_strings",
    [
        ["Tensor<[1, 32, 512, 512]> self = ?", "number negative_slope = 0.1"],
        ["Tensor<[1, 64, 256, 256]> self = ?", "number negative_slope = 0.1"],
        ["Tensor<[1, 32, 256, 256]> self = ?", "number negative_slope = 0.1"],
        ["Tensor<[1, 128, 128, 128]> self = ?", "number negative_slope = 0.1"],
        ["Tensor<[1, 64, 128, 128]> self = ?", "number negative_slope = 0.1"],
        ["Tensor<[1, 256, 64, 64]> self = ?", "number negative_slope = 0.1"],
        ["Tensor<[1, 128, 64, 64]> self = ?", "number negative_slope = 0.1"],
        ["Tensor<[1, 512, 32, 32]> self = ?", "number negative_slope = 0.1"],
        ["Tensor<[1, 256, 32, 32]> self = ?", "number negative_slope = 0.1"],
        ["Tensor<[1, 1024, 16, 16]> self = ?", "number negative_slope = 0.1"],
...
def test_aten(device, input_strings, input_var_only_native, input_var_check_accu, input_var_check_ttnn):
    metric = {
        "opname": "aten.leaky_relu.default",
    ...
    input_args, input_kwargs, status = render_metric_string_list_to_input_args_kwargs(
        "aten.leaky_relu.default", input_strings
    )
    if status == False:
