...
class AtenModule(torch.nn.Module):
    ...
    def forward(self, *args, **kwargs):
        return torch.ops.aten._adaptive_avg_pool2d.default(*args, **kwargs)
...
def teardown_module(module):
    print(metrics)
    save_pickle(metrics, "metrics-autogen-op/vgg13_bn", "aten._adaptive_avg_pool2d.default")


@pytest.mark.parametrize("input_strings", [["Tensor<[1, 512, 7, 7]> self = ?", "List[int] output_size = [7, 7]"]])
def test_aten(device, input_strings, input_var_only_native, input_var_check_accu, input_var_check_ttnn):
    metric = {
        "opname": "aten._adaptive_avg_pool2d.default",
    ...
    input_args, input_kwargs, status = render_metric_string_list_to_input_args_kwargs(
        "aten._adaptive_avg_pool2d.default", input_strings
    )
    if status == False:
