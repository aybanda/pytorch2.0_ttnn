...
class AtenModule(torch.nn.Module):
    ...
    def forward(self, *args, **kwargs):
        return torch.ops.aten.split_with_sizes.default(*args, **kwargs)
...
def teardown_module(module):
    print(metrics)
    save_pickle(metrics, "metrics-autogen-op/retinanet_resnet50_fpn_v2", "aten.split_with_sizes.default")


@pytest.mark.parametrize(
    "input_strings",
    [
        ["Tensor<[1, 163206, 91]> self = ?", "List[int] split_sizes = [122400, 30600, 7650, 1989, 567]", "int dim = 1"],
        ["Tensor<[1, 163206, 4]> self = ?", "List[int] split_sizes = [122400, 30600, 7650, 1989, 567]", "int dim = 1"],
        ["Tensor<[163206, 4]> self = ?", "List[int] split_sizes = [122400, 30600, 7650, 1989, 567]"],
    ],
)
def test_aten(device, input_strings, input_var_only_native, input_var_check_accu, input_var_check_ttnn):
    metric = {
        "opname": "aten.split_with_sizes.default",
    ...
    input_args, input_kwargs, status = render_metric_string_list_to_input_args_kwargs(
        "aten.split_with_sizes.default", input_strings
    )
    if status == False:
