...
class AtenModule(torch.nn.Module):
    ...
    def forward(self, *args, **kwargs):
        return torch.ops.aten.fill.Scalar(*args, **kwargs)
...
def teardown_module(module):
    print(metrics)
    save_pickle(metrics, "metrics-autogen-op/retinanet_resnet50_fpn_v2", "aten.fill.Scalar")


@pytest.mark.parametrize(
    "input_strings",
    [
        ["Tensor<[]> self = ?", "number value = 8"],
        ["Tensor<[]> self = ?", "number value = 16"],
        ["Tensor<[]> self = ?", "number value = 32"],
        ["Tensor<[]> self = ?", "number value = 61"],
        ["Tensor<[]> self = ?", "number value = 64"],
        ["Tensor<[]> self = ?", "number value = 114"],
        ["Tensor<[]> self = ?", "number value = 120"],
    ],
)
def test_aten(device, input_strings, input_var_only_native, input_var_check_accu, input_var_check_ttnn):
    metric = {
        "opname": "aten.fill.Scalar",
    ...
    m = AtenModule()
    input_args, input_kwargs, status = render_metric_string_list_to_input_args_kwargs("aten.fill.Scalar", input_strings)
    if status == False:
