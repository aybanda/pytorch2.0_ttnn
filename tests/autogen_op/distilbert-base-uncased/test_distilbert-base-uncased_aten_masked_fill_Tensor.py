...
class AtenModule(torch.nn.Module):
    ...
    def forward(self, *args, **kwargs):
        return torch.ops.aten.masked_fill.Tensor(*args, **kwargs)
...
def teardown_module(module):
    print(metrics)
    save_pickle(metrics, "metrics-autogen-op/distilbert-base-uncased", "aten.masked_fill.Tensor")


@pytest.mark.parametrize(
    "input_strings", [["Tensor<[1, 12, 16, 16]> self = ?", "Tensor<[1, 12, 16, 16]> mask = ?", "Tensor value = ?"]]
)
def test_aten(device, input_strings, input_var_only_native, input_var_check_accu, input_var_check_ttnn):
    metric = {
        "opname": "aten.masked_fill.Tensor",
    ...
    input_args, input_kwargs, status = render_metric_string_list_to_input_args_kwargs(
        "aten.masked_fill.Tensor", input_strings
    )
    if status == False:
