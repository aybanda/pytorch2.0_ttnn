...
class AtenModule(torch.nn.Module):
    ...
    def forward(self, *args, **kwargs):
        return torch.ops.aten.remainder.Scalar(*args, **kwargs)
...
def teardown_module(module):
    print(metrics)
    save_pickle(metrics, "metrics-autogen-op/GPT-2", "aten.remainder.Scalar")


@pytest.mark.parametrize("input_strings", [["Tensor<[1]> self = ?", "number other = 7"]])
def test_aten(device, input_strings, input_var_only_native, input_var_check_accu, input_var_check_ttnn):
    metric = {
        "opname": "aten.remainder.Scalar",
    ...
    input_args, input_kwargs, status = render_metric_string_list_to_input_args_kwargs(
        "aten.remainder.Scalar", input_strings
    )
    if status == False:
